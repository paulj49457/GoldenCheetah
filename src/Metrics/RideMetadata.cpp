    ...
    for(const SummaryKeywordDefinition& keyword : getSummaryKeywords()) {
        ...
        if ((keyword.name == "*") || (keyword.name == summaryField)) {
            ...
            for (const QString& fieldName : keyword.summaryFields) {
                ...
                QString value;
                GcFieldType fieldType = GcFieldType::NO_FIELD_SET;
                ...
                if (sp.isMetric(internalfieldName)) {
                    if (sp.rideMetric(fieldName)->isRelevantForRide(rideItem)) {
                        fieldType = GcFieldType::FIELD_DOUBLE;
...
bool MetadataXMLParser::endElement( const QString&, const QString&, const QString &qName)
{
    ...
    else if(qName == "fieldtype") field.type = static_cast<GcFieldType>(buffer.trimmed().toInt());

...
static const QColor ITEM_WARNING_COLOR = QColor(255, 170, 0);
...
class AbstractEqItem : public QObject
{
    public:
        ...
        virtual bool itemWarning() const { return false; };
        ...
        virtual bool parseXml(uint32_t version, QXmlStreamReader& reader) = 0;
...
class EqItem : public AbstractEqItem
{
    public:
        ...
        // equipment use time windows
        bool isWithin(const QDate& actDate) const;
        bool isWithin(const QStringList& rideEqLinkNameList, const QDate& actDate) const;
        bool rangeIsValid() const;
        bool allEqLinkNamesCompleterVals() const;

        // equipment wear status
        bool overDistance() const { return ((repDistance_ !=0) && (totalDistance_ > repDistance_)); }
        bool overElevation() const { return ((repElevation_ !=0) && (totalElevation_ > repElevation_)); }
        bool overDate() const { return (repDateSet_ && (QDate::currentDate() > repDate_)); }
        bool itemWarning() const override { return !rangeIsValid() || !allEqLinkNamesCompleterVals() || overDistance() || overElevation() || overDate(); }
        ...
        void setRepDistance(double repDistance) { repDistance_ = repDistance; }
        double getRepDistance() const { return repDistance_; }
        void setRepElevation(double repElevation) { repElevation_ = repElevation; }
        double getRepElevation() const { return repElevation_; }

        void setRepDateSet(bool repDateSet) { repDateSet_ = repDateSet; }
        bool getRepDateSet() const { return repDateSet_; }
        void setRepDate(const QDate& repDate) { repDate_ = repDate; }
        QDate getRepDate() const { return repDate_; }

        void setDisplayTotalDistance(bool displayTotalDistance) { displayTotalDistance_ = displayTotalDistance; }       
        bool displayTotalDistance() const { return displayTotalDistance_; }

        void clearEqTimeWindows() { eqLinkUseList_.clear(); }
        void addEqTimeWindow(const EqTimeWindow& eqLinkUse) { eqLinkUseList_.push_back(eqLinkUse); }
        const QVector<EqTimeWindow>& getEqTimeWindows() const { return eqLinkUseList_; }
        ...
        bool parseXml(uint32_t version, QXmlStreamReader& reader) override;

...
ImportChartDialog::ImportChartDialog(Context *context, const QList<QMap<QString,QString>>& list, QWidget *parent) : QDialog(parent), context(context), list(list)
{
    ...
    for(int i=0; i<list.count(); i++) {
        ...
        else if (view == EquipmentView::internalName) view = tr(EquipmentView::userName);
...
ImportChartDialog::importClicked()
{
    ...
    for(int i=0; i<list.count(); i++) {
        ...
        if (static_cast<QCheckBox*>(table->cellWidget(i,0))->isChecked()) {
            ...
            if (view == tr(EquipmentView::userName)) { viewType=GcViewType::VIEW_EQUIPMENT; context->mainWindow->selectEquipment(); }

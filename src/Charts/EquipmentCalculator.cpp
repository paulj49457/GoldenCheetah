...
EquipCalculator::EquipCalculator(const MainWindow* mainWindow, EquipmentOverviewWindow* eqOverviewWindow) :
...
EquipCalculator::recalculateTiles()
{
    ...
    for (ChartSpaceItem* item : spaceItems_) {
        ...
        static_cast<CommonEquipmentItem*>(item)->resetForRecalc();
    }
...
EquipCalculator::recalculateEq(RideItem* rideItem)
{
    ...
    for (ChartSpaceItem* item : spaceItems_) {
        ...
        default: {
            qDebug() << "Unsupported tile type:" << item->type << " in Equipment Calculator";
        } break;
        }
    }
}

...
EquipmentOverviewItemConfig::EquipmentOverviewItemConfig(ChartSpaceItem* item) :
    OverviewItemConfig(item), eqLinkCompleter(nullptr)
{
    ...
    foreach(FieldDefinition field, GlobalContext::context()->rideMetadata->getFields()) {
        if (field.name == "EquipmentLink") {
            ...
            eqLinkCompleter = field.getCompleter(this, nullptr);
            break;
        }
    }
...
EquipmentOverviewItemConfig::setWidgets()
{
    ...
    case OverviewItemType::EQ_ITEM:
    {
        ...
        displaytotal->setCurrentIndex(mi->displayTotalDistance() ? 1 : 0);
        ...
        replaceElevation->setText(QString::number(mi->getRepElevation(), 'f', EQ_DECIMAL_PRECISION));
        layout->setRowVisible(nonGCElevation, showEqItemElevationWids);
        layout->setRowVisible(replaceElevation, showEqItemElevationWids);
        replaceDistance->setText(QString::number(mi->getRepDistance(), 'f', EQ_DECIMAL_PRECISION));
        replaceDateSet->setText((mi->getRepDateSet()) ? "reset" : "set");
        replaceDate->setVisible(mi->getRepDateSet());
        if (mi->getRepDateSet()) replaceDate->setDate(mi->getRepDate());

        // clear the table
        eqTimeWindows->setRowCount(0);

        for (int tableRow = 0; tableRow < mi->getEqTimeWindows().size(); tableRow++) {

            eqTimeWindows->insertRow(tableRow);
            const EqTimeWindow& eqUse = mi->getEqTimeWindows()[tableRow];
    ...
    case OverviewItemType::EQ_ITEM:
    {
        ...
        for (int tableRow = 0; tableRow < eqTimeWindows->rowCount(); tableRow++) {
            ...
            if (eqLinkName != "") { // don't accept time windows without any eqLinkName text
                ...
                mi->addEqTimeWindow(EqTimeWindow(eqLinkName, startSet, startDate, endSet, endDate));
            }
        }

        mi->setRepDistance(replaceDistance->text().toDouble());
        mi->setRepElevation(replaceElevation->text().toDouble());

        mi->setRepDateSet(replaceDateSet->text() == "reset");
        if (mi->getRepDateSet()) mi->setRepDate(replaceDate->date());
    ...
    : CommonEqItem(parent, name, equipmentRef)
{
    ...
    configwidget_ = new EquipmentOverviewItemConfig(this);
    configwidget_->hide();
...
EquipmentItem::itemPaint(QPainter* painter, const QStyleOptionGraphicsItem*, QWidget*) {
    ...
    double mainDisplayValue = eqItem->displayTotalDistance() ? eqItem->getTotalDistance() : eqItem->getTotalElevation();
    ...
    if (eqItem->itemWarning()) {
        painter->setPen(alertColor_);
    } else if (eqItem->isWithin(QDate::currentDate())) {
    ...
    QString totalUnits = eqItem->displayTotalDistance() ? distUnits : elevUnits;
    ...
    for (const auto& eqUse : eqItem->getEqTimeWindows()) {
    ...
    if (eqItem->getRepDistance() != 0) {
        if (eqItem->overDistance()) painter->setPen(alertColor_);
        rowY += ROWHEIGHT;
        addNotesOffset = true;
        painter->drawText(QRectF(ROWHEIGHT, rowY, rowWidth, rowHeight),
            QString(tr("Replace dist: ")) + QString("%L1").arg(eqItem->getRepDistance(), 0, 'f', 0) + distUnits);
        if (eqItem->overDistance()) painter->setPen(textColor_);
    }

    if (showElevationWids && (eqItem->getRepElevation() != 0)) {
        if (eqItem->overElevation()) painter->setPen(alertColor_);
        rowY += ROWHEIGHT;
        addNotesOffset = true;
        painter->drawText(QRectF(ROWHEIGHT, rowY, rowWidth, rowHeight),
            QString(tr("Replace elev: ")) + QString("%L1").arg(eqItem->getRepElevation(), 0, 'f', 0) + elevUnits);
        if (eqItem->overElevation()) painter->setPen(textColor_);
    }

    if (eqItem->getRepDateSet()) {
        if (eqItem->overDate()) painter->setPen(alertColor_);
        rowY += ROWHEIGHT;
        addNotesOffset = true;
        painter->drawText(QRectF(ROWHEIGHT, rowY, rowWidth, rowHeight),
            QString(tr("Replace date: ")) + eqItem->getRepDate().toString("->d MMM yy"));
        if (eqItem->overDate()) painter->setPen(textColor_);
    }

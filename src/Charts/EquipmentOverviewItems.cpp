...
EquipmentOverviewItemConfig::setWidgets()
{
    ...
    {
        bool showElevationWids(dynamic_cast<EquipmentOverviewWindow*>(item->parent->window)->isShowElevation());
        layout->setRowVisible(nonGCElevation, showElevationWids);
        layout->setRowVisible(replaceElevation, showElevationWids);

        bool showNotesWids(dynamic_cast<EquipmentOverviewWindow*>(item->parent->window)->isShowNotes());
        layout->setRowVisible(notes, showNotesWids);
...
CommonEquipmentItem::CommonEquipmentItem(ChartSpace* parent, const QString& name, const QUuid& equipmentRef) :
    ChartSpaceItem(parent, name), tileDisplayHeight_(ROWHEIGHT * 5), equipmentRef_(equipmentRef)
{
    setShowEdit(true);
}
...
void
CommonEquipmentItem::configChanged(qint32 cfg) {
...
EquipmentItem::EquipmentItem(ChartSpace* parent, const QString& name, const QUuid& equipmentRef)
    : CommonEquipmentItem(parent, name, equipmentRef)
{
    // equipment item
...
EquipmentItem::configChanged(qint32 cfg) {
    ...
    if (cfg & CONFIG_APPEARANCE) {
        ...
        CommonEquipmentItem::configChanged(cfg);
...
EquipmentItem::itemPaint(QPainter* painter, const QStyleOptionGraphicsItem*, QWidget*) {
    ...
    QFont selected;
    switch (dynamic_cast<EquipmentOverviewWindow*>(parent->window)->isTextSize()) {
    ...
    if (showNotesWids && (!scrollableDisplayText_.isEmpty())) {
...
EquipmentSummary::EquipmentSummary(ChartSpace* parent, const QString& name,
    ...
    EquipmentSummary(parent, name, QUuid::createUuid())
{
    setEqLinkName(eqLinkName);
...
void
EquipmentSummary::setEqLinkName(const QString& eqLinkName)
{
...
void
EquipmentSummary::resetForRecalc()
{
...
EquipmentHistory::EquipmentHistory(ChartSpace* parent, const QString& name, const QVector<EqHistoryEntry>& eqHistory, const bool sortMostRecentFirst) :
    EquipmentHistory(parent, name, QUuid::createUuid())
{
...
EquipmentNotes::EquipmentNotes(ChartSpace* parent, const QString& name, const QString& notes) :
    EquipmentNotes(parent, name, QUuid::createUuid())
{
    notes_ = notes;

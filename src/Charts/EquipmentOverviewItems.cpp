    ...
    OverviewItemConfig(item), eqLinkCompleter(nullptr)
{
    ...
    if (item->type == OverviewItemType::EQ_ITEM) {
        ...
        connect(displaytotal, SIGNAL(currentIndexChanged(int)), this, SLOT(dataChanged()));
...
EquipmentOverviewItemConfig::setWidgets()
{
    ...
    case OverviewItemType::EQ_ITEM:
    {
        ...
        nonGCElevation->setEnabled(showEqItemElevationWids);
        replaceElevation->setEnabled(showEqItemElevationWids);
#else
...
#if QT_VERSION < 0x060400
        notes->setEnabled(showEqItemNotesWids);
#else

...
OverviewWindow::OverviewWindow(Context *context, MainWindow *mainWindow, OverviewScope scope, bool blank) :
    GcChartWindow(mainWindow), context(context), configured(false), blank(blank)
{
    ...
    space = new ChartSpace(context, mainWindow, scope, this);
    ...
    if (context != nullptr) connect(context, SIGNAL(refreshEnd()), space, SLOT(refresh()));
    if (context != nullptr) connect(context, SIGNAL(estimatesRefreshed()), space, SLOT(refresh()));
...
badconfig:
    ...
    for(const QJsonValue &val : CHARTS) {
        ...
        ChartSpaceItem *add = setTileConfig(obj, type, name, datafilter, order, column, span, deep);
...
{
    QSize gcWindowSize = item->parent->mainWindow->size();
    QPoint gcWindowPosn = item->parent->mainWindow->pos();
